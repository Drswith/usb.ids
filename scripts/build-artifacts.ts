...
function readDataset(): UsbDatasetV2 {
  if (!fs.existsSync(srcPath)) {
    console.error('Missing usb.ids.json — run pnpm run fetch-usb-ids or regenerate from usb.ids first.')
    process.exit(1)
  }
  const raw = JSON.parse(fs.readFileSync(srcPath, 'utf8')) as unknown
  if (isDatasetV2(raw))
    return raw
  if (isV1VendorShape(raw))
    return v1ToDatasetV2(raw)
...
function main(): void {
  ...
  if (fs.existsSync(versionPath)) {
    try {
      ...
      fs.writeFileSync(path.join(outDir, 'usb.ids.version.json'), `${JSON.stringify(ver, null, 2)}\n`)
    }
    catch (e) {
