...
describe("resolveUpstreamMeta", () => {
  it("uses header when present", () => {
    expect(resolveUpstreamMeta("# Version: 2026.01.02\n", null, true)).toEqual({
      version: "2026.01.02",
      date: null,
    });
  });
  ...
  it("throws when strict and raw usb.ids has no Version line", () => {
    expect(() => resolveUpstreamMeta("aaaa  Vendor\n", null, true)).toThrow(/missing # Version/);
  });
  ...
  it("throws JSON path without existing upstream", () => {
    expect(() => resolveUpstreamMeta("{}", null, false)).toThrow(/requires upstreamVersion/);
  });
