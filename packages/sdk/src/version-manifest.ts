...
import { legacyReleaseToUpstream, legacyV10FetchTimestampToUpstream } from "./manifest-ui";
import { formatDateTime } from "./parser/datetime";
import { parseUsbIdsHeader } from "./parser/upstream-header";

export { legacyReleaseToUpstream, legacyV10FetchTimestampToUpstream } from "./manifest-ui";
...
export function normalizeVersionInfo(raw: unknown): VersionInfo | null {
  ...
  let upstreamVersion: string | null =
    typeof r.upstreamVersion === "string" ? r.upstreamVersion : null;
  if (!upstreamVersion) {
    upstreamVersion =
      legacyReleaseToUpstream(releaseVersion) ?? legacyV10FetchTimestampToUpstream(releaseVersion);
  }
...
export function resolveUpstreamMeta(
  ...
  if (header.version) return { version: header.version, date: header.date };
