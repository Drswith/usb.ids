  ...
  describe("async functions", () => {
    describe("getVendors", () => {
      ...
      it("should filter vendors by function", async () => {
        const vendors = await api.getVendors((vendor: UsbVendor) =>
          vendor.name.toLowerCase().includes("apple"),
        );
        expect(vendors.length).toBeGreaterThanOrEqual(0);
        if (vendors.length > 0) {
          expect(vendors[0].name.toLowerCase()).toContain("apple");
    ...
    describe("getVendor", () => {
      ...
      it("should filter by function", async () => {
        const vendor = await api.getVendor((v: UsbVendor) =>
  ...
  describe("pure functions", () => {
    describe("filterVendors", () => {
      ...
      it("should filter vendors by function", () => {
        const vendors = api.filterVendors(mockUsbData, (vendor: UsbVendor) =>
          vendor.name.toLowerCase().includes("apple"),
        );
        expect(vendors.length).toBeGreaterThanOrEqual(0);
        if (vendors.length > 0) {
    ...
    describe("filterDevices", () => {
      ...
      it("should filter devices by function", () => {
        ...
        if (firstDeviceId) {
          const devices = api.filterDevices(
            firstVendor,
            (device: UsbDevice) => device.devid === firstDeviceId,
          );
          expect(devices).toHaveLength(1);
